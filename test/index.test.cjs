...
test('preserves the popup redirect URI for Apollo final login handling', async () => {
  ...
  // Model the callback request Apollo passes to the custom authentication handler.
  ...
  // Complete plugin authentication before Apollo processes its own redirect state.
  const user =
    await handler.login(
      callbackRequest,
    )
  ...
  // Model Apollo parsing the returned OAuth state after authentication succeeds.
  const apolloState =
    JSON.parse(
      callbackRequest.query.state,
    )

  // Apollo must still receive exactly the popup URI supplied on the first leg.
  assert.equal(
